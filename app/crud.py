...
def get_competitors(db: Session, skip: int = 0, limit: int = 100):
    """获取竞品列表（含关联商品名）"""
    competitors = db.query(models.Competitor).order_by(models.Competitor.created_at.desc()).offset(skip).limit(limit).all()
...
def get_competitor(db: Session, competitor_id: int):
    """获取单个竞品"""
    competitor = db.query(models.Competitor).filter(models.Competitor.id == competitor_id).first()
    if competitor:
...
def get_competitors_by_product(db: Session, product_id: int):
    """获取某商品的所有竞品"""
    return db.query(models.Competitor).filter(models.Competitor.product_id == product_id).all()
...
def create_competitor(db: Session, competitor_data: dict):
    ...
    db_competitor.product_name = db_competitor.product.name if db_competitor.product else ""
    return db_competitor


def update_competitor(db: Session, competitor_id: int, competitor_data: dict):
    """更新竞品"""
    db_competitor = db.query(models.Competitor).filter(models.Competitor.id == competitor_id).first()
...
def get_profit_records(db: Session, skip: int = 0, limit: int = 50):
    """获取利润测算历史"""
    return db.query(models.ProfitRecord).order_by(models.ProfitRecord.created_at.desc()).offset(skip).limit(limit).all()


def get_profit_record(db: Session, record_id: int):
    """获取单条利润记录"""
    return db.query(models.ProfitRecord).filter(models.ProfitRecord.id == record_id).first()


def delete_profit_record(db: Session, record_id: int):
    """删除利润记录"""
    db_record = db.query(models.ProfitRecord).filter(models.ProfitRecord.id == record_id).first()
    if not db_record:
        return False
    db.delete(db_record)
    db.commit()
...
def get_dashboard_stats(db: Session):
    ...
    # 库存预警：库存小于 10 的商品
    low_inventory = db.query(func.count(models.Product.id)).filter(
        models.Product.inventory < 10, models.Product.status == "active"
    ).scalar() or 0

    # 高风险利润测算
    high_risk = db.query(func.count(models.ProfitRecord.id)).filter(
        models.ProfitRecord.risk_level == "high"
    ).scalar() or 0
...
def get_product_status_distribution(db: Session):
    """商品状态分布"""
    results = db.query(models.Product.status, func.count(models.Product.id)).group_by(models.Product.status).all()
...
def get_recent_profit_records(db: Session, limit: int = 5):
    """获取最近的利润测算"""
    return db.query(models.ProfitRecord).order_by(models.ProfitRecord.created_at.desc()).limit(limit).all()
